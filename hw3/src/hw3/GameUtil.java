...
public class GameUtil {
	...
	public TilePosition generateRandomTilePosition(int[][] grid, Random rand, Direction lastMove) {
		...
		int zero = rand.nextInt(zeroArr.length);

		if (lastMove == Direction.UP) {
			TilePosition tile = new TilePosition(grid.length - 1, zeroArr[zero], generateRandomTileValue(rand));
			return tile;
		}
		else if (lastMove == Direction.DOWN) {
			TilePosition tile = new TilePosition(0, zeroArr[zero], generateRandomTileValue(rand));
			return tile;
		}
		else if (lastMove == Direction.LEFT) {
			TilePosition tile = new TilePosition(zeroArr[zero], grid.length - 1, generateRandomTileValue(rand));
			return tile;
		}
		else
		{
			TilePosition tile = new TilePosition(zeroArr[zero], 0, generateRandomTileValue(rand));
			return tile;
		}
	}
		...
		for (int i = 0; i < arr.length - 1; i++) {
			if (arr[i] == 0 && arr[i + 1] != 0) {
				...
				if (mergeValues(arr[i], arr[i + 1]) > 0) {
					...
					arr[i + 1] = 0;
					list.add(new Move(i + 1, i, arr[i + 1], arr[i], arr[i + 1] + arr[i]));
				}
			}
		}
