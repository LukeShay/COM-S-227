...
/**
 * A partial implementation of the Game interface for Tetris-like
 * falling block games. Subclasses must implement the
 * determinePositionsToCollapse() and determineScore() methods.
 */
public abstract class AbstractGame implements Game {
	...
	/**
	 * A grid whose positions may be occupied by the blocks the current
	 * falling shape or blocks that can no longer be moved. Unoccupied
	 * cells are null.
	 */
	private Block[][] grid;

	/**
	 * Status of the game after each invocation of step(), as described in
	 * the GameStatus documentation.
	 */
	private GameStatus gameStatus;

	/**
	 * Generator for new shapes. (The BasicGenerator implementation will
	 * uniformly select one of the six shape types).
	 */
	private Generator generator;

	/**
	 * State variable indicating which blocks are to be deleted when the
	 * status is COLLAPSING. The implementation maintains the invariant
	 * that positionsToCollapse.size() is nonzero if and only if
	...
	/**
	 * Returns a list of locations for all cells that form part of a
	 * collapsible set. This list may contain duplicates.
	 * 
	...
	/**
	 * Deletes the blocks at the indicated positions and shifts blocks
	 * above them down. Only blocks lying within a column above a deleted
	 * block are shifted down. This method does not update the game state
	 ...
	 * 
	 * @return true if the current shape can be shifted down, false
	 *         otherwise
	 */
	 ...
	 * modify the game state.
	 * 
	 * @return true if the current shape can be shifted right, false
	 *         otherwise
	 */
	 ...
	 * modify the game state.
	 * 
	 * @return true if the current shape can be shifted left, false
	 *         otherwise
	 */
	 ...
	 * modify the game state.
	 * 
	 * @return true if the current shape can be transformed, false
	 *         otherwise
	 */
	...
	/**
	 * Determines whether the given shape overlaps with the occupied cells
	 * of the grid, or extends beyond the sides or bottom of the grid. (A
	 * shape in its initial position MAY extend above the grid.)
	 *
	 * @param t
	 *            a shape
	 * @return true if the cells of the given shape extend beyond the
	 *         sides or bottom of the grid or overlap with any occupied
	 *         cells of the grid
	 */
